...
class plgSystemJPrCron extends JPlugin {
    ...
    function onAfterRender() {
        ...
        if ($params->get('params.cronenable'))
            if ($app->isSite()) {
                require_once( JPATH_PLUGINS . "/system/jprcron/jprcron/CronParser.class.php");
                ########## GET CONFIG VARIABLES ##########################
                $_CONFIG['dlog'] = $params->get('params.dlog');
                $_CONFIG['email'] = $params->get('params.email');
                $_CONFIG['cemail'] = $params->get('params.cemail');
                #########################################################
                ...
                foreach ($data as $value) {
                    ...
                    if ($value->last_run == false) {
                        mail($_CONFIG['email'], "Alert ", "A cron task was failed, " . ($value->task) . " crashed when we tried to run it.", 'JPrCron Daemon');
                    }
                    ...
                    if ($cron->calcLastRan($value->unix_mhdmd)) {
                        ...
                        if (date('Y-m-d H:i:s', $cron->getLastRanUnix()) > $value->ran_at) {
                            ...
                            if ($_CONFIG['cemail']) {
                                ...
                                mail($_CONFIG['email'], $subject, $logdata, 'JPrCron Daemon');
                            }
                        }
                    }
                }
