...
<form action="<?php echo $uri = JFactory::getURI()->toString() ; ?>" method="post" name="adminForm" id="adminForm">
	<fieldset id="filter-bar">
		<div class="filter-search fltlft pull-left">
			...
			<button class="btn" type="button" onclick="document.id('search_index').value='';this.form.submit();"><?php echo JText::_('JSEARCH_FILTER_CLEAR'); ?></button>
		</div>
	...
	<table class="table table-striped">
		...
		<?php foreach ($this->items as $i => $item) :
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td>
					<a href="<?php echo $item->a_url; ?>" class="hasTooltip" title="網址::<?php echo htmlentities($item->a_url) ?>">
				...
				<td>
					<?php
						if($item->a_downloaded=='0000-00-00 00:00:00' && !$item->a_error ) {
							...
							echo '<span class="text-error" style="color: #B94A48;">下載失敗，<a href="javascript:void(0)" title="'.$item->a_error.'" class="hasTooltip">錯誤訊息</a></span>' ;
						}else {
							echo '<span class="text-success" style="color: #468847;">下載成功，時間：<br />'.$item->a_downloaded.'</span>' ;
						}
					?>
				</td>
				...
				<td>
					...
					<?php elseif($item->referenced) : ?>
						<img src="templates/bluestork/images/admin/tick.png" alt="是" />
					<?php else: ?>
						<img src="templates/bluestork/images/admin/publish_x.png" alt="否" />
					<?php endif; ?>
				</td>
