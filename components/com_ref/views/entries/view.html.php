...
class RefViewEntries extends RefView
{
	...
	public function display($tpl = null)
	{
		...
		foreach( $this->items as &$item ):
			$item->link = JRoute::_("index.php?option=com_ref&view=entry&id={$item->a_id}&alias={$item->a_alias}&catid={$item->a_catid}");
			...
			$results = $dispatcher->trigger('onContentPrepare', array ('com_ref.entry', &$item, &$this->params, 0));

			$results = $dispatcher->trigger('onContentAfterTitle', array('com_ref.entry', &$item, &$item->params, 0));
			$item->event->afterDisplayTitle = trim(implode("\n", $results));

			$results = $dispatcher->trigger('onContentBeforeDisplay', array('com_ref.entry', &$item, &$item->params, 0));
			$item->event->beforeDisplayContent = trim(implode("\n", $results));

			$results = $dispatcher->trigger('onContentAfterDisplay', array('com_ref.entry', &$item, &$item->params, 0));
