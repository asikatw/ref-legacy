...
class RefModelpage extends JModelAdmin
{
	...
	public function getResultPage() {
		...
		foreach( $urls as $url ):
			...
			if(!$erron) {
				...
				// update url 
				$url->loading 		= 0 ;
				$url->downloaded	= JFactory::getDate( 'now' , JFactory::getConfig()->get('offset') )->toSQL() ;
				$db->updateObject('#__rp_url_index', $url , 'id' ) ;
	...
	public function parseResultPage() {
		...
		foreach( $pages as $k => $page ):
			...
			// get params
			$args['professors_titles'] 	= RF::getConfig('professors_titles', '');
			$args['course_terms'] 		= RF::getConfig('course_terms', '');
			$args['reference_terms'] 	= RF::getConfig('reference_terms', '');
			$args['author_terms'] 		= RF::getConfig('author_terms', '');
			...
			$args['professors_names'] 	= (array)json_decode($page->professors_name );
			$args['txt'] 				= RefReader::read($page->file_path);
			$args['units'] 				= RF::getConfig('professors_titles_units', '');
			
			// parse and detect
			$result = RF::_('detector.detect', $page->database, $args) ;
			RF::_('detector.saveResult', $page->task_id, $result) ;
